...
class BitStructTest(unittest.TestCase):
    ...
    def test_align_right(self):
        ...
        for offset, expected in datas:
            packed = bytearray(2)
            pack_into('u2', packed, offset, 3)
            self.assertEqual(packed, expected)

        for offset, expected in datas:
            packed = bytearray(2)
            pack_into('u2', packed, offset, 3, align_right=True)
            self.assertEqual(packed, expected)

        packed = bytearray(b'\xff\xff\xff')
        pack_into('u4p4u4p4u4', packed, 0, 1, 2, 3, fill_padding=False, align_right=True)
        self.assertEqual(packed, b'\xf1\xf2\xf3')

        packed = bytearray(b'\xff\xff\xff')
        pack_into('u4p4u4p4u4', packed, 0, 1, 2, 3, fill_padding=True, align_right=True)
        self.assertEqual(packed, b'\xf1\x02\x03')

        packed = bytearray(2)

        with self.assertRaises(Error) as cm:
            pack_into('u17', packed, 0, 1, align_right=True)
        ...
        with self.assertRaises(Error) as cm:
            packed = bytearray(b'\x00')
            pack_into('b1t24', packed, 0, False, align_right=True)
